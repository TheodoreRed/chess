...
class Game:
    ...
    def handle_input(self):
        ...
        while not current_pos:
            current_pos_input = input(
                "What piece do you want to move? (format: a1) "
            ).lower()
            ...
            if not piece:
                ...
                print(
                    "Try again: wrong team. You do not control the piece at {}".format(
                        current_pos_input
                    )
                )
                current_pos = None
        ...
        while not new_pos:
            new_pos_input = input(
                "Where do you want to move this piece? (format: a1) "
            ).lower()
        ...
        if self.board.move(current_pos, new_pos):
            ...
            print(
                "Moved {} from {} to {}".format(
                    self.board.get_piece(new_pos), current_pos_input, new_pos_input
                )
            )
    ...
    def convert_input_to_coordinates(input):
        ...
        if not col_letter.isalpha():
            print(
                "Try again: column needs to be a letter. Got {} instead".format(
                    col_letter
                )
            )
            return None
        ...
        if col_letter not in LETTER_TO_COL.keys():
            print(
                "Try again: column letter needs to be between a-h. Got {} instead".format(
                    col_letter
                )
            )
            return None
        ...
        if not row_number.isnumeric():
            print(
                "Try again: row needs to be a number. Got {} instead".format(row_number)
            )
            return None
        ...
        if row_number < 1 or row_number > 8:
            print(
                "Try again: row needs to be between 1-8. Got {} instead".format(
                    row_number
                )
            )
            return None
    ...
    def finish_turn(self):
        self.current_team = (
            Team.WHITE if self.current_team == Team.BLACK else Team.BLACK
        )
        print("--------")
